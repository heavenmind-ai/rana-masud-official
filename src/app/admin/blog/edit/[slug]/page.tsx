...
export default function AdminBlogEditPage() {
  ...
  useEffect(() => {
    async function fetchPost() {
      try {
        const res = await fetch(`/api/blog/${encodeURIComponent(slugParam)}`);
        if (!res.ok) throw new Error("Failed to load blog post");
        const data = await res.json();
        
        setTitle(data.title || "");
        setDescription(data.description || data.frontmatter?.summary || "");
        setImage(data.frontmatter?.image || "");
  ...
  const handleSave = async () => {
    if (!title.trim()) {
      setErrorMsg("Title is a required field.");
      return;
    }
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className="ml-3 text-sm text-white/60">Loading blog post details...</span>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between border-b border-white/5 pb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-white/50 mt-1">Make modifications to your published article.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 flex flex-col gap-5">
          <div className="glass-card p-6 flex flex-col gap-4 border border-white/10">
            ...
            <div className="flex flex-col gap-1.5">
              <label className="text-[10px] text-white/40 font-bold uppercase">URL Slug (Cannot edit)</label>
              <input
                type="text"
                value={decodeURIComponent(slugParam)}
                readOnly
                disabled
                className="px-3 py-2 rounded bg-white/5 border border-white/5 text-white/40 text-xs font-mono select-all cursor-not-allowed outline-none"
              />
            </div>
